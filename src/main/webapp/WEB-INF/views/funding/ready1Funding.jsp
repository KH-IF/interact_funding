    ...
    <div id="makerGuide" class="modal" tabindex="-1" role="dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body" data-spy="scroll"  style="height: 500px;">
                ...
                보다 성공적인 펀딩을 위한 다양한 부가 서비스를 자유롭게 선택하실 수 있도록
                 ...
                 모든 요금제는 PG사 결제 대행 수수료를 포함하고 있으며, 제공되는 서비스에 따라 수수료율이 부과되는 방식입니다.<br><br>
              	<p class="modelPTage">1) Light 요금제</p>
                 	<p>Light는 중개 서비스에 PG 수수료를 포함한 요금제입니다. (기존 9.4% → 9.0%)<br>
	                    ...
	                    <span class="FundingInfoModal_strong__Qr7rx">
	                        이프 스쿨 수강, 데이터 플러스, 새소식 알림 서비스, 펀딩 인증마크, 제휴 서비스
	                    </span>를 제공합니다.
                   	</p><br>
             	<p class="modelPTage">2) Basic 요금제</p>
           		...
           		<p class="modelPTage">3) Pro 요금제</p>
                    <p>Pro 요금제는 Basic 요금제가 제공하는 모든 서비스를 포함하며 
                          ...
                          공간이프 쇼룸</a>을 제공</span>합니다. (14%, 신설 요금제)
                    </p><br>
                <p class="modelPTage">4) Expert 요금제</p>
                	<p>Expert 요금제는 Pro 요금제가 제공하는 모든 서비스를 포함하며 추가적으로 광고 대행 서비스, PD 컨설팅, 에디터 피드백을 제공합니다. (별도 협의, 신설 요금제)
                	</p><br>
                	
             	3. 적용 일자<br>2021년 5월 3일 09시 이후, 메이커 스튜디오 [제출하기]를 클릭한 건부터 적용<br><br>
                * 상기 수수료에는 VAT가 포함되어 있지 않습니다.<br><br>
                각 서비스에 대한 자세한 안내사항은&nbsp;
                <a class="FundingInfoModal_link__GsFYL" href="https://www.wadiz.kr/link/funding_plan_newsBoard_202104">공지사항</a>
                을 참고해주시기 바랍니다.<br>
                펀딩 수수료 관련 문의사항은 info@if.kr 로 문의주시면 답변드리겠습니다.
                <br><br>고맙습니다.<br>이프 드림<br><br>
            </div>
            <div class="modal-footer d-flex justify-content-between">
                ...
                <button id="close" type="button" class="border-0 text-info bg-white font-weight-bold" data-dismiss="modal">닫기</button>
            </div>
        </div>
        </div>
    </div>
		...
		<!-- 	 <div class="leftB rounded p-3">
			...
			<c:if test="${funding.status != true && loginMember.phone != null}">
				<div class="d-flex flex-row  pb-4">
					<button name="disabledFT" id="checkSMSPhone" class="btn btn-info btn-lg mr-4 " role="button" style="width: 200px;" onclick="finalSubmit()" >제출하기</button>
					<button name="disabledFT" id="REcheckSMSPhone" class="btn btn-info btn-lg" role="button" style="width: 200px;" onclick="location.href='${pageContext.request.contextPath}/funding/checkSMS';" >번호인증 다시하기</button>
			...
			<c:if test="${funding.status == true}">
				<div class="d-flex flex-row align-items-center">
					<c:if test="${writeMap.ratePlanCode == 'OK' && writeMap.basicInfo == 'OK'&& writeMap.story == 'OK' && writeMap.reward == 'OK'}">
						<button name="disabledFT" id="finalYSubmit" class="btn btn-info btn-lg mr-4" role="button" style="width: 200px;" onclick="finalYSubmit()" >수정하기</button>			
						<button name="disabledFT" id="REcheckSMSPhone" class="btn btn-info btn-lg" role="button" style="width: 200px;" onclick="location.href='${pageContext.request.contextPath}/funding/checkSMS';" >번호인증 다시하기</button>	
						<p class="text-muted" >(사용자 전화번호가 변경되고 제출됩니다.)</p>
					</c:if>
				<!-- 최종 제출이 되었는데 수정해서 미완성일때  -->
					<c:if test="${writeMap.ratePlanCode != 'OK' || writeMap.basicInfo != 'OK' || writeMap.story != 'OK' || writeMap.reward != 'OK'}">
						<button id="finalNSubmit" class="btn btn-info btn-lg " role="button" style="width: 200px;" onclick="finalNSubmit()">수정하기</button>
					</c:if>
				</div>
			</c:if>
