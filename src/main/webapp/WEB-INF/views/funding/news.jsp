...
<script>
	...
	<section>
		...
		<form id="newsFrm" name="newsWriteContainer"
			action="${pageContext.request.contextPath}/funding/newsEnroll.do"
			method="post">
			<input id="fundingNo" type="hidden" name="fundingNo"
				value="${funding.fundingNo}" /> 
			<input id="writerNo" type="hidden"
				name="writerNo" value="${loginMember.memberNo}" />
			<div class="newsWriteContainerDiv" id="newsWriteContainerDiv">
				<div class="newsWriteView" id="newsWriteView">
					...
					<input type="text" name="title" id="newsTitle" placeholder="제목을 입력하세요." />
					<!-- 썸머노트 -->
					<div class="col-12 col-md-12">
						<br />
						<textarea name="content" class="summernote" placeholder="내용 입력"
	...
	$("#newsWriteBtn").click(function(){
		console.log(111);
		 if($(newsWriteView).css('display') == 'none'){
		       //열어주어라
		       document.getElementById("newsWriteView").style.display='block'
		    //그렇지 않은 모든 경우라면??
		    }else{
		       //닫아주어라
		       document.getElementById("newsWriteView").style.display='none'
		    }
	});
	...
	$("#newsFrm").submit(e =>{
		  ...
		  $.ajax({
		      url : "${pageContext.request.contextPath}/funding/newsEnroll.do",
		      ...
		      success(data) {
					...
					.then(function(){
						...
						/* $("[class=yh-table]").append('<tr>' +
							'<td><a href="${pageContext.request.contextPath}/funding/newsView.do?fundingNo='+news.fundingNo+'&no='+news.no+'">' +
							'<p><small>이벤트</small></p>' +
							'<p>'+news.title+'</p>' +
