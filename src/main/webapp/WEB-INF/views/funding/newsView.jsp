...
<div id="funding_top_title_wrapper_wrapper">
	<div id="funding_top_title_wrapper">
		<div id="funding_top_title">
			<img
				src="${pageContext.request.contextPath }/resources/upload/${funding.attachment.renamedFilename}"
				id="funding_top_title_image">
		</div>
		<div id="funding_top_title_div">
			<c:if test="${funding.categoryCode == 'C1'}">
				<span class="category_code_span">테크·가전</span>
			</c:if>
			<c:if test="${funding.categoryCode == 'C2'}">
				<span class="category_code_span">푸드</span>
			</c:if>
			<c:if test="${funding.categoryCode == 'C3'}">
				<span class="category_code_span">여행</span>
			</c:if>
			<c:if test="${funding.categoryCode == 'C4'}">
				<span class="category_code_span">스포츠</span>
			</c:if>
			<c:if test="${funding.categoryCode == 'C5'}">
				<span class="category_code_span">게임·취미</span>
			</c:if>
			<c:if test="${funding.categoryCode == 'C6'}">
				<span class="category_code_span">모임</span>
			</c:if>
			<c:if test="${funding.categoryCode == 'C7'}">
				<span class="category_code_span">반려동물</span>
			</c:if>
			<c:if test="${funding.categoryCode == 'C8'}">
				<span class="category_code_span">기부·후원</span>
			</c:if>
			<br /> ${funding.title}
		</div>
	</div>
...
<div id="funding_tap_wrapper">
	<div id="funding_tap">
		<ol>
			<li><a
				href="${pageContext.request.contextPath }/funding/fundingDetail?fundingNo=${funding.fundingNo}">스토리</a>
			</li>
			<li><a
				href="${pageContext.request.contextPath }/funding/news.do?fundingNo=${funding.fundingNo}">새소식</a>
			</li>
			<li><a
...
<div id="funding_main">
	...
	<!-- 남은날계산용 -->
	<fmt:parseNumber value="${funding.startDate.time / (1000*60*60*24)}"
		integerOnly="true" var="strDate"></fmt:parseNumber>
	<fmt:parseNumber value="${funding.DDay.time / (1000*60*60*24)}"
		integerOnly="true" var="endDate"></fmt:parseNumber>

	<div id="funing_main_right_div">
		<div id="funing_main_right_div_1">
			<div id="funding_detail_dday_div">${endDate - strDate}일남았습니다</div>
			...
			<div id="funding_detail_goal_percent_div">
				<fmt:formatNumber value="${funding.nowAmount / funding.goalAmount}"
					type="percent" />
				달성
			</div>
			<!-- 8500% 달성 -->

			<div id="funding_detail_now_amount_div">현재
				${funding.nowAmount}원 펀딩중</div>
			<!-- 444444 원 펀딩 -->
			<div id="funding_detail_supporter_div">현재
				${fundingParticipationCount}명의 서포터</div>
			<!-- 3333명의 서포터funding_participation -->
			<input id="funding_button" type="button" value="펀딩하기"
				onclick="location.href='${pageContext.request.contextPath}/funding/fundingReward?fundingNo=	${funding.fundingNo}';"
				class="btn btn-info" />
			<button type="button" id="funding_detail_like_button"
				class="btn btn-outline-secondary" onclick="like_controll()">
				<!--하트  -->
				<span id="icon_heart" style="color: red;"><i
					class="far fa-heart"></i></span>
				<!--좋아요숫자  -->
				<span id="funding_detail_like_count">${funding.likeCount}</span>
			</button>
			<button type="button" value="1:1 채팅" id="funding_detail_chat_button"
		...
		<span id="funding_detail_like_count"></span> <span id="maker_span">메이커
			정보</span>
		<!--메이커정보 (해당 펀딩의 작성자정보) -->
		<div id="funing_main_right_div_2">

			<div id="funding_detail_maker_logo_name_div">
				<div id="funding_detail_maker_name" data-toggle="modal"
					data-target="#modalSendMsg" data-tomemberno="${funding.writerNo}"
					onclick="msgSetting(this)">${wirterName}</div>
			</div>
			<div id="funding_detail_maker_phone">
				메이커 연락처 <br />${funding.phone}</div>
			<input type="button" value="메이커에게 문의하기"
				id="funding_detail_maker_chat_button" class="btn btn-info"
		...
		<c:forEach var="reward" items="${reward}">
			<div id="funing_main_right_div_3"
				class="funding_main_reward_choice_div"
				data-choice="${reward.rewardNo}">
				${reward.price}원 펀딩 <br /> ${reward.title}!! <br />
				${reward.content} 혜택 <br /> 배송비 <br /> ${reward.shippingPrice} <br />
				리워드 발송 시작일 <br /> ${reward.rewardNo} 리워드넘버 <br />
				${reward.limitAmount} 개 남음!! <br /> ㅁㅁㅁㅁ 펀딩완료
			</div>
		</c:forEach>
